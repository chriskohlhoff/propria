...
//
// * Sender - A function (or function object) that takes a Receiver as one of
//   its arguments
//
// * Receiver - An object that determines how and where the results of a Sender
//   will be received
//
// * Communication between Sender and Receiver is unidirectional and consists
//   of a set of signals, where each signal has a call Signature. The set of
//   possible signals is defined as a variadic parameter pack of Signatures.
//
// * receiver_traits<> - A traits type used to determine concretely how a
//   Receiver handles a given set of signals, specified as Signatures.
//
// * Handler - a function object, produced by a Receiver, to handle the set
//   of signals generated by a Sender. It is callable with each of the given
//   Signatures.
//
// * SenderLauncher - A function object used to launch the underlying operation
//   associated with a Sender.
//
// * receiver_traits<>::connect() - Called by a Sender. Passed the SenderLauncher,
//   the Receiver, and an optional list of arguments to be passed back to the
//   SenderLauncher. The Receiver is responsible for calling the SenderLauncher
//   with its Handler and the optional arguments. The Receiver gets to choose
//   when the SenderLauncher is called, which allows the model to support eager,
//   lazy, and other hybrid models of execution.
//
// * connect<>() - A helper function used by a Sender to simplify the call to
//   receiver_traits<>::connect().
//
// Executors:
//
// * Executor - provides a member function execute(), where execute() is a
//   Sender.
//
// * executor<> - polymorphic wrapper for Executors
//
//------------------------------------------------------------------------------
...
struct archetypal_execute_receiver
{
...
template <class E, class R = detail::archetypal_execute_receiver>
concept bool Executor = requires(const E& e, R&& r)
{
  e.execute(std::forward<R>(r));
};
...
public:
  ...
  template <Receiver<void(executor<>)> R>
  auto execute(R&& r) const
  {
    return execution::connect<void(executor)>(
        [](Handler<void(executor<>)> h, const executor& ex)
        {
          if (ex.blocking_ == blocking.always)
          {
            function<decltype(h)> f(std::move(h));
            ex.target_fns_->blocking_execute(ex, f);
          }
          else
          {
            ex.target_fns_->execute(ex,
                std::make_unique<function<decltype(h)>>(std::move(h)));
          }
        },
        std::forward<R>(r),
        *this);

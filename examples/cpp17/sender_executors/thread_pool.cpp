...
public:
  ...
  public:
    ...
    template <execution::Receiver<void(sub_executor_type)> R>
    auto execute(R&& r) const
    {
      return execution::connect<void(sub_executor_type)>(
          [](
              execution::Handler<void(sub_executor_type)> h,
              queue* q
            )
          {
            std::unique_lock<std::mutex> lock(q->mutex_);
            q->functions_.emplace_front(new function<decltype(h)>(std::move(h)));
  ...
  public:
    ...
    auto execute(R&& r) const
    {
      return execution::connect<void(sub_executor_type)>(
          ...
          {
            auto qn = p->next_queue_++ % p->thread_queues_.size();
            queue& q = p->thread_queues_[qn];
            std::unique_lock<std::mutex> lock(q.mutex_);
            q.functions_.emplace_back(new function<decltype(h)>(std::move(h)));
            q.condition_.notify_one();
          },
          std::forward<R>(r),
          &pool_
